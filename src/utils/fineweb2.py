...
#TESTI MOLTO BUONI SOLO 500 GOOD I BAD VANNO PRESI ALTROVE
import json
from datasets import load_dataset
from tqdm import tqdm
import time

def collect_good_only(output_file, total_target=500):
    """Estrae SOLO testi di alta qualità - scorrevoli e ben strutturati."""
    
    import os
    os.environ['HF_HUB_READ_TIMEOUT'] = '60'
    os.environ['HF_HUB_ETAG_TIMEOUT'] = '60'
    
    print("--- Inizio download (SOLO BUONI TESTI) ---")
    ...
    print(f"Target: {total_target} testi di qualità\n")
    ...
    try:
        for example in tqdm(ds, desc="Raccolta testi"):
            ...
            # === CRITERI STRINGENTI PER "BUONO" ===
            
            # Lunghezza ragionevole (300-5000 caratteri)
            if text_len < 300 or text_len > 5000:
                continue
            
            # Deve avere buona diversità di vocabolario
            words = text.split()
            if len(words) > 0:
                unique_ratio = len(set(words)) / len(words)
                if unique_ratio < 0.60:  # Poco vocabolario = ripetitivo
                    continue
            
            # Punteggiatura ordinata (non troppa, non troppo poca)
            punctuation_chars = ".,!?;:-)()\""
            punctuation_density = sum(1 for c in text if c in punctuation_chars) / (text_len + 1)
            if punctuation_density < 0.02 or punctuation_density > 0.20:
                continue
            
            # Deve avere frasi ben strutturate
            sentences = [s.strip() for s in text.split('.') if s.strip()]
            if len(sentences) < 2:  # Almeno 2 frasi
                continue
            
            avg_sentence_len = text_len / len(sentences)
            if avg_sentence_len < 20 or avg_sentence_len > 120:  # Frasi "normali"
                continue
            
            # Whitespace pulito
            whitespace_ratio = sum(1 for c in text if c.isspace()) / (text_len + 1)
            if whitespace_ratio < 0.10 or whitespace_ratio > 0.30:  # Deve avere spazi ragionevoli
                continue
            
            # NO caratteri strani/corrotti
            special_ratio = sum(1 for c in text if ord(c) > 127 and c not in 'àèéìòùÀÈÉÌÒÙ') / (text_len + 1)
            if special_ratio > 0.03:
                continue
            
            # NO spam (maiuscole consecutive)
            consecutive_caps = sum(1 for i in range(len(text)-1) if text[i].isupper() and text[i+1].isupper())
            caps_ratio = consecutive_caps / (text_len + 1)
            if caps_ratio > 0.05:
                continue
            ...
            # ✅ ACCETTA - È UN BUON TESTO
            entry = {
                "id": f"good_{good_count}",
                "text": text,
                "metadata": {"label": "good", "length": len(text)}
            }
            ...
            if processed % 200 == 0:
                pct = (good_count / total_target * 100) if total_target > 0 else 0
                print(f"  Processati: {processed} | Good salvati: {good_count}/{total_target} ({pct:.1f}%)")
            
            # Fermati quando hai abbastanza
            if good_count >= total_target:
                print(f"\n✓ Target raggiunto!")
                break
        ...
        print(f"\nChiusura file completata.")
        print(f"Processati totali: {processed}")
        print(f"Good salvati: {good_count}")
        print(f"File salvato in: {output_file}")

if __name__ == "__main__":
    collect_good_only("../../data/train/training_data.jsonl", total_target=500)